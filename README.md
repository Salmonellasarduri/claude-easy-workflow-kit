...
Claude Code での開発を、無理なく「いつもの流れ」に乗せるためのワークフローキットです。

「まず考える」「実装する」「動作確認する」「レビューして保存する」といった一連の流れを、`/command` で回しやすくします。

新規プロジェクトでも、既存プロジェクトでも使えます。
Claude Code を普段使っていて、「毎回の進め方をもう少し安定させたい」と感じている人向けです。
